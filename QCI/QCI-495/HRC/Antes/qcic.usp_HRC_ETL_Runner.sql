...
BEGIN
	...
	DECLARE @SystemName nvarchar(100) = 'HRC'
			,@SystemNameInt int = 3
			,@MaxDate date
	DROP TABLE IF EXISTS [qcic].[HRC_ETL_Runner_Tracker] -- CHANGE TO [qcic]
	SELECT TimeStamp = GETDATE(),TableCompleted = convert(nvarchar(255),'Start'),RecordCount = 0 INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
	...
	******************/
	...
	BEGIN TRANSACTION
		...
		MERGE [qcids].[Dim_Host_SCD] T -- CHANGE TO [qcids]
		...
		WHEN NOT MATCHED BY SOURCE AND T.SystemNameInt = @SystemNameInt
			THEN DELETE;
	COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- CHANGE to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Dim_Host_SCD]',RecordCount = count(*) FROM [qcids].[Dim_Host_SCD] -- change to [qcids]
   ...
   BEGIN TRANSACTION
		...
		MERGE [qcids].[Dim_Player_SCD] T -- CHANGE TO [qcids]
    ...
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] --CHANGE TO [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Dim_Player_SCD]',RecordCount = count(*) FROM [qcids].[Dim_Player_SCD] -- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
    ...
    BEGIN TRANSACTION
		...
		MERGE [qcids].[Dim_PlayerDistanceToCasino_SCD] T -- CHANGE TO [qcids]
    ...
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- CHANGE TO [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Dim_PlayerDistanceToCasino_SCD]',RecordCount = count(*) FROM [qcids].[Dim_PlayerDistanceToCasino_SCD] -- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
    ...
    BEGIN TRANSACTION
		...
		MERGE [qcids].[Dim_PlayerLoyalty_SCD] T --change to [qcids]
    ...
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Dim_PlayerLoyalty_SCD]',RecordCount = count(*) FROM [qcids].[Dim_PlayerLoyalty_SCD]-- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
	...
	--DECLARE @SystemNameInt int = 3
     BEGIN TRANSACTION
		...
		MERGE [qcids].[Dim_PlayerHost_SCD] T -- change to [qcids]
    ...
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Dim_PlayerHost_SCD]',RecordCount = count(*) FROM [qcids].[Dim_PlayerHost_SCD] -- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
    ...
    BEGIN TRANSACTION
		...
		MERGE [qcids].[Dim_PlayerCredit_SCD] T -- change to [qcids]
    ...
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Dim_PlayerCredit_SCD]',RecordCount = count(*) FROM [qcids].[Dim_PlayerCredit_SCD]-- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
   ...
   BEGIN TRANSACTION
		...
		MERGE [qcids].[Dim_PlayerCreditElectronic_SCD] T -- change to [qcids]
    ...
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Dim_PlayerCreditElectronic_SCD]',RecordCount = count(*) FROM [qcids].[Dim_PlayerCreditElectronic_SCD] -- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
    ...
    SELECT *
	    FROM [qcic].[udf_HRC_ETL08](@MaxDate,@days );
	--DECLARE @SystemNameInt int = 3, @MaxDate DATE
    BEGIN TRANSACTION
		--SELECT @MaxDate = MAX(GamingDate) FROM [qcids].[Fact_PlayerDeviceRatingDay] WITH (NOLOCK) WHERE SystemNameInt = @SystemNameInt
		--SELECT @MaxDate = ISNULL(@MaxDate,'2000-01-01')
		DELETE FROM [qcids].[Fact_PlayerDeviceRatingDay] WHERE SystemNameInt = @SystemNameInt AND GamingDate >= DATEADD(DAY,-10,@MaxDate) -- change to [qcids]
		INSERT INTO [qcids].[Fact_PlayerDeviceRatingDay] SELECT *  FROM #HRC_ETL08 -- WHERE  GamingDate >= DATEADD(DAY,-10,@MaxDate) >> inside udf_HRF_ETL08  --change first to [qcids], 
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Fact_PlayerDeviceRatingDay]',RecordCount = count(*) FROM [qcids].[Fact_PlayerDeviceRatingDay] -- change to [qcids]	
																													WHERE SystemNameInt = @SystemNameInt
	...
	SELECT * 
		FROM [qcic].[udf_HRC_ETL09]();
	--DECLARE @SystemNameInt int = 3
    BEGIN TRANSACTION
		DELETE FROM [qcids].[App_DeviceManufacturerGroup] WHERE SystemNameInt = @SystemNameInt -- change to [qcids]
		INSERT INTO [qcids].[App_DeviceManufacturerGroup] SELECT * FROM #HRC_ETL09 --change to [qcids]
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[App_DeviceManufacturerGroup]',RecordCount = count(*) FROM [qcids].[App_DeviceManufacturerGroup] -- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
	...
	SELECT * 
	FROM  [qcic].[udf_HRC_ETL10]();
	--DECLARE @SystemNameInt int = 3
    BEGIN TRANSACTION
		DELETE FROM [qcids].[App_DeviceGameTypeGroup] WHERE SystemNameInt = @SystemNameInt -- change to [qcids]
		INSERT INTO [qcids].[App_DeviceGameTypeGroup] SELECT * FROM #HRC_ETL10 -- change first to [qcids]
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[App_DeviceGameTypeGroup]',RecordCount = count(*) FROM [qcids].[App_DeviceGameTypeGroup] -- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
	...
	SELECT * 
	FROM   [qcic].[udf_HRC_ETL11]();

	--DECLARE @SystemNameInt int = 3
    BEGIN TRANSACTION
		DELETE FROM [qcids].[App_DeviceDenominationGroup] WHERE SystemNameInt = @SystemNameInt --change to [qcids]
		INSERT INTO [qcids].[App_DeviceDenominationGroup] SELECT * FROM #HRC_ETL11  --change first to [qcids]
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[App_DeviceDenominationGroup]',RecordCount = count(*) FROM [qcids].[App_DeviceDenominationGroup] -- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
	...
	SELECT * 
	FROM [qcic].[udf_HRC_ETL12]();
	--DECLARE @SystemNameInt int = 3, @MaxDate DATE
    BEGIN TRANSACTION
		--SELECT @MaxDate = MAX(PrizeStartDateTime) FROM [qcids].[Fact_PlayerPrize] WITH (NOLOCK) WHERE SystemNameInt = @SystemNameInt
		--SELECT @MaxDate = ISNULL(@MaxDate,'2000-01-01')
		DELETE FROM [qcids].[Fact_PlayerPrize] WHERE SystemNameInt = @SystemNameInt AND PrizeStartDateTime >= DATEADD(DAY,-10,@MaxDate) -- change to [qcids]
		INSERT INTO [qcids].[Fact_PlayerPrize] SELECT * FROM #HRC_ETL12 WHERE PrizeStartDateTime >= DATEADD(DAY,-10,@MaxDate) -- change first o [qcids]
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Fact_PlayerPrize]',RecordCount = count(*) FROM [qcids].[Fact_PlayerPrize] -- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
	...
	SELECT * FROM [qcic].[udf_HRC_ETL14]();
	--DECLARE @SystemNameInt int = 3, @MaxDate DATE
    BEGIN TRANSACTION
		...
		MERGE [qcids].[Fact_PlayerEvaluationCurrent] T --change to [qcids]
    ...
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] -- change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Fact_PlayerEvaluationCurrent]',RecordCount = count(*) FROM [qcids].[Fact_PlayerEvaluationCurrent] -- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
	...
	SELECT @MaxDate = ISNULL(@MaxDate,'2000-01-01')
	 ...
	 SELECT * 
		FROM [qcic].[udf_HRC_ETL15](@MaxDate , @days) ;
	--DECLARE @SystemNameInt int = 3, @MaxDate DATE
    BEGIN TRANSACTION
	--	SELECT @MaxDate = MAX(GamingDate) FROM [qcids].[Fact_DeviceDay] WITH (NOLOCK) WHERE SystemNameInt = @SystemNameInt
	--	SELECT @MaxDate = ISNULL(@MaxDate,'2000-01-01')
		DELETE FROM [qcids].[Fact_DeviceDay] WHERE SystemNameInt = @SystemNameInt AND GamingDate >= DATEADD(DAY,-10,@MaxDate) --change to [qcids]
		INSERT INTO [qcids].[Fact_DeviceDay] SELECT * FROM #HRC_ETL15 --<< inside  udf WHERE GamingDate >= DATEADD(DAY,-10,@MaxDate) --change to [qcids]
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker] --change to [qcic]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Fact_DeviceDay]',RecordCount = count(*) FROM [qcids].[Fact_DeviceDay] -- change to [qcids]
		WHERE SystemNameInt = @SystemNameInt
