...
End
CREATE table #HRC_ETL01   (
        SystemName NVARCHAR(100), SystemNameInt INT, CasinoGroupID NVARCHAR(100), CasinoGroupIDInt INT, HostID NVARCHAR(100),
...
CREATE TABLE #HRC_ETL02  (
   SystemName  NVARCHAR(100), SystemNameInt INT, PlayerID NVARCHAR(100), PlayerIDBigInt BIGINT, Account NVARCHAR(100), Title NVARCHAR(20), 
  ...
  AltTelephone2Type NVARCHAR(20), AltTelephone2 NVARCHAR(20), 
  AltEmail NVARCHAR(255), BirthDate DATE, IsPrivacy INT, IsMailToAlt INT, IsNoMail INT, IsReturnMail INT, IsVIP INT, IsLostAndFound INT, IsCreditAccount INT, 
...
SELECT * 
FROM [test].[udf_HRC_ETL03](@SystemName, @SystemNameInt ) ;
...
SELECT * 
FROM [test].[udf_HRC_ETL04](@SystemName, @SystemNameInt ) ;
...
CREATE TABLE #HRC_ETL05  ( 
    SystemName  NVARCHAR(100),  SystemNameInt INT,  CasinoID NVARCHAR(100), CasinoIDInt INT, PlayerID NVARCHAR(100), 
    PlayerIDBigInt BIGINT , HostID NVARCHAR(100), HostIDBigInt BIGINT,  PlayerHostType NVARCHAR(100),  StartDateTime DATETIME, 
    EndDateTime DATETIME,  IsCurrent INT,  InsertDateTime DATETIME
); 

INSERT INTO #HRC_ETL05
SELECT * 
FROM [test].[udf_HRC_ETL05](@SystemName, @SystemNameInt ) ;
...
CREATE table #HRC_ETL06  (
 SystemName NVARCHAR(100),  SystemNameInt INT, CasinoID  NVARCHAR(100), CasinoIDInt  INT,
  PlayerID NVARCHAR(100),  PlayerIDBigInt BIGINT,  CreditLimitInt100 INT, IsActiveCredit INT,  
  SetupCasinoID NVARCHAR(100), Note NVARCHAR(4000), StartDateTime DATETIME, EndDateTime DATETIME, 
  IsCurrent INT , InsertDateTime DATETIME     						

)
INSERT INTO #HRC_ETL06
SELECT * 
FROM [test].[udf_HRC_ETL06](@SystemName, @SystemNameInt ) ;

--DECLARE @SystemName nvarchar(100) = 'HRC'	,@SystemNameInt int = 3
...
SELECT * FROM [test].[udf_HRC_ETL07](@SystemName, @SystemNameInt ) ;
...
CREATE TABLE #HRC_ETL08 ( SystemName NVARCHAR(100), [SystemNameInt] INT,  [CasinoID]  NVARCHAR(100), [CasinoIDInt]  INT
      ,[DeviceID]  NVARCHAR(100), [DeviceIDBigInt] BIGINT,
	    [PlayerID]  NVARCHAR(100), [PlayerIDBigInt] BIGINT  
      ,[GamingDate]  DATE	, 
	  [StartDateTime]  DATETIME , [EndDateTime]  DATETIME,
	  [CoinInInt100]  BIGINT, [CoinOutInt100]  BIGINT ,
	  [ActualWinWithFreeplayInt100]  INT
      ,[ActualWinNetFreeplayInt100]  INT ,
	  [NumberGamesPlayed]  INT, 
	  [JackpotInt100]  INT,
	  [FreeplayInt100]  INT
      ,[TheoWinWithFreeplayInt100]  INT,
	  [TheoWinNetFreeplayInt100]  INT,
	  [TimePlayedInt100]  INT, 
	  [CashInInt100]  BIGINT
      ,[ChipsInInt100]  BIGINT,
	  [CreditInInt100]  BIGINT, 
	  [WalkedWithInt100]  BIGINT, 
	  [PlayerZipCode]  VARCHAR(100)
      ,[PlayerGender]  VARCHAR(50), [PlayerEnrollmentDate]  DATE, [PlayerBirthDate]  DATE, 
	  [PlayerAddressDistance]  FLOAT
      ,[GameTheme]  NVARCHAR(100), [GameThemeCount]  NVARCHAR(100), [GameTheoHold]  FLOAT, 
	  [DeviceTheoHoldPercent]  FLOAT
      ,[DeviceManufacturer]  NVARCHAR(200),
	  [Denomination]  FLOAT,
	  [DeviceGameType]   NVARCHAR(200), 
	  [DeviceCabinetType]  NVARCHAR(100)
      ,[DeviceCabinetModel]  NVARCHAR(200), 
	  [DeviceLocation]  NVARCHAR(20), 
	  [DeviceLocationArea]  NVARCHAR(20), 
	  [DeviceLocationBank]  NVARCHAR(20)	
      ,[DeviceLocationStand]  NVARCHAR(20),
	  [DeviceStartDate]  DATE, [DeviceSerial]  NVARCHAR(50)
      ,[DeviceGameClass]  INT, [DeviceLeaseFeeInt100]  INT
      ,[DeviceLeaseAttributes]  NVARCHAR(4000)	, [DeviceExtraAttributes]  NVARCHAR(4000)
      ,[IsSlotDevice]  INT , [IsTableDevice]  INT, [IsPokerDevice]  INT, [IsOtherDevice]  INT			       
      ,[IsActiveDevice]  INT , [DeviceLocationBigInt]  BIGINT, [InsertDateTime]  DATETIME );

INSERT INTO  #HRC_ETL08
SELECT *
FROM [test].[udf_HRC_ETL08](@SystemName, @SystemNameInt ) ;
...
SELECT * 
FROM [test].[udf_HRC_ETL09](@SystemName , @SystemNameInt);
...
CREATE TABLE #HRC_ETL10 ( SystemName NVARCHAR(100), SystemNameInt INT, OriginalGameType NVARCHAR(100), 
                            GameTypeCleaned NVARCHAR(100) , GameTypeGroup NVARCHAR(100), GameTypeShortName NVARCHAR(100), 
                            GameTypeSummary NVARCHAR(100),  InsertDateTime DATETIME ) ;
INSERT INTO #HRC_ETL10
SELECT * 
FROM  [test].[udf_HRC_ETL10](@SystemName , @SystemNameInt);
...
End
CREATE TABLE #HRC_ETL11  (SystemName NVARCHAR(100), SystemNameInt INT, OriginalDenomination NVARCHAR(100), 
                    DenominationCleaned NVARCHAR(100), DenominationSort NVARCHAR(100), DenominationGroup NVARCHAR(100) , DenominationGroupSort NVARCHAR(100), 
                    InsertDateTime DATETIME );
INSERT INTO #HRC_ETL11 
SELECT * 
FROM   [test].[udf_HRC_ETL11](@SystemName , @SystemNameInt);
...
SELECT * 
FROM [test].[udf_HRC_ETL12](@SystemName , @SystemNameInt);
...
End
CREATE TABLE #HRC_ETL14( SystemName NVARCHAR(100), SystemNameInt INT, PlayerID NVARCHAR(100), 
                                                 PlayerIDBigInt BIGINT, CasinoGroupID NVARCHAR(100), CasinoGroupIDInt INT, 
                                                 EvaluationScoreInt100 INT, InsertDateTime DATETIME ) ;
INSERT INTO #HRC_ETL14
SELECT * FROM [test].[udf_HRC_ETL14](@SystemName , @SystemNameInt);
...
SELECT * FROM [test].[udf_HRC_ETL15](@SystemName , @SystemNameInt);

--Begin legacy store procedure
	BEGIN TRANSACTION
   ...
   BEGIN TRANSACTION
    ...
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Dim_Player_SCD]',RecordCount = count(*) FROM [test].[Dim_Player_SCD]
		WHERE SystemNameInt = @SystemNameInt
...
--DECLARE @SystemNameInt int = 3
    ...
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Dim_PlayerCredit_SCD]',RecordCount = count(*) FROM [test].[Dim_PlayerCredit_SCD]
		WHERE SystemNameInt = @SystemNameInt
   ...
   BEGIN TRANSACTION
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[Fact_PlayerDeviceRatingDay] SELECT * FROM #HRC_ETL08 WHERE GamingDate >= DATEADD(DAY,-10,@MaxDate)
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[test].[Fact_PlayerDeviceRatingDay]',RecordCount = count(*) 
		        FROM [test].[Fact_PlayerDeviceRatingDay]
		        WHERE SystemNameInt = @SystemNameInt

	--DECLARE @SystemNameInt int = 3
    BEGIN TRANSACTION
		DELETE FROM [test].[App_DeviceManufacturerGroup] WHERE SystemNameInt = @SystemNameInt
		INSERT INTO [test].[App_DeviceManufacturerGroup] SELECT * FROM #HRC_ETL09
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[App_DeviceGameTypeGroup] SELECT * FROM #HRC_ETL10
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[App_DeviceDenominationGroup] SELECT * FROM #HRC_ETL11
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[Fact_PlayerPrize] SELECT * FROM #HRC_ETL12 WHERE PrizeStartDateTime >= DATEADD(DAY,-10,@MaxDate)
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[Fact_DeviceDay] SELECT * FROM #HRC_ETL15 WHERE GamingDate >= DATEADD(DAY,-10,@MaxDate)
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
