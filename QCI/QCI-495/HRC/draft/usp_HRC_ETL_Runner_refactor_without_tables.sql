...
SELECT TimeStamp = GETDATE(),TableCompleted = convert(nvarchar(255),'Start'),RecordCount = 0 INTO [test].[HRC_ETL_Runner_Tracker]
	...
	BEGIN TRANSACTION
		...
		USING (SELECT *  FROM [test].[udf_HRC_ETL01]( )) AS S
   ...
   BEGIN TRANSACTION
		...
		USING ( SELECT * FROM [test].[udf_HRC_ETL02]( )) S
...
--DECLARE @SystemNameInt int = 3
    BEGIN TRANSACTION
		...
		USING ( SELECT * FROM [test].[udf_HRC_ETL03]( ) ) S
  ...
  BEGIN TRANSACTION
		...
		USING ( SELECT * FROM [test].[udf_HRC_ETL04]()  )  S
		ON T.SystemNameInt = S.SystemNameInt AND T.PlayerIDBigInt = S.PlayerIDBigInt AND T.StartDateTime = S.StartDateTime AND T.IsCurrent = S.IsCurrent
		WHEN MATCHED AND 
	...
	--DECLARE @SystemNameInt int = 3
     BEGIN TRANSACTION
		...
		USING ( SELECT * FROM [test].[udf_HRC_ETL05]( )  )  S
    ...
    BEGIN TRANSACTION
		...
		USING ( SELECT * FROM [test].[udf_HRC_ETL06]( ) )  S
		ON  T.SystemNameInt = S.SystemNameInt AND T.CasinoIDInt = S.CasinoIDInt AND T.PlayerIDBigInt = S.PlayerIDBigInt AND T.StartDateTime = S.StartDateTime AND T.IsCurrent = S.IsCurrent
		WHEN MATCHED AND 
   ...
   BEGIN TRANSACTION
		...
		USING ( SELECT * FROM [test].[udf_HRC_ETL07]( ) ) S
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[Fact_PlayerDeviceRatingDay] SELECT *  FROM [test].[udf_HRC_ETL08](@MaxDate,@days )-- << inside udf WHERE GamingDate >= DATEADD(DAY,-10,@MaxDate)
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[test].[Fact_PlayerDeviceRatingDay]',RecordCount = count(*) FROM [test].[Fact_PlayerDeviceRatingDay]
		WHERE SystemNameInt = @SystemNameInt
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[App_DeviceManufacturerGroup] SELECT * FROM [test].[udf_HRC_ETL09]()
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[App_DeviceGameTypeGroup] SELECT * FROM [test].[udf_HRC_ETL10]()
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[App_DeviceDenominationGroup] SELECT * FROM [test].[udf_HRC_ETL11]()
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[Fact_PlayerPrize] SELECT * FROM [test].[udf_HRC_ETL12]() WHERE PrizeStartDateTime >= DATEADD(DAY,-10,@MaxDate)
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		USING (SELECT * FROM [test].[udf_HRC_ETL14]())  S
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[Fact_DeviceDay] SELECT * FROM [test].[udf_HRC_ETL15](@MaxDate , @days) -- << inside udf  WHERE GamingDate >= DATEADD(DAY,-10,@MaxDate)
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
