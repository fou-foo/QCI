...
--DECLARE @SystemNameInt int = 3
   ...
   BEGIN TRANSACTION
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[Fact_PlayerDeviceRatingDay] SELECT * FROM #HRC_ETL08 -- WHERE  GamingDate >= DATEADD(DAY,-10,@MaxDate) >> inside udf_HRF_ETL08 
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [test].[Fact_DeviceDay] SELECT * FROM #HRC_ETL15 --<< inside  udf WHERE GamingDate >= DATEADD(DAY,-10,@MaxDate)
    COMMIT TRANSACTION
		INSERT INTO [test].[HRC_ETL_Runner_Tracker]
