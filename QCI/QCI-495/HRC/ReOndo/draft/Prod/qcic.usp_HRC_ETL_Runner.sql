...
Notes:			Runs the ETLs for HRC System to build the UDM Connection Layer as well as the base UDM Data Science Layer Tables
				...
				Requires usp_HRC_ETL08_Fact_PlayerDeviceRatingDay, usp_HRC_ETL12_Fact_PlayerPrize 
				              usp_HRC_ETL15_Fact_DeviceDay to be created third
...
SELECT TimeStamp = GETDATE(),TableCompleted = convert(nvarchar(255),'Start'),RecordCount = 0 INTO [qcic].[HRC_ETL_Runner_Tracker]
	BEGIN TRANSACTION
		...
		USING [qcic].[uvw_HRC_ETL01_Dim_Host_SCD] S
   ...
   BEGIN TRANSACTION
		...
		USING [qcic].[uvw_HRC_ETL02_Dim_Player_SCD] S
    ...
    BEGIN TRANSACTION
		...
		USING [qcic].[uvw_HRC_ETL03_Dim_PlayerDistanceToCasino_SCD] S
  ...
  BEGIN TRANSACTION
		...
		USING [qcic].[uvw_HRC_ETL04_Dim_PlayerLoyalty_SCD] S
		ON T.SystemNameInt = S.SystemNameInt AND T.PlayerIDBigInt = S.PlayerIDBigInt AND T.StartDateTime = S.StartDateTime AND T.IsCurrent = S.IsCurrent
		WHEN MATCHED AND 
    ...
    COMMIT TRANSACTION
     ...
     BEGIN TRANSACTION
		...
		USING [qcic].[uvw_HRC_ETL05_Dim_PlayerHost_SCD] S
    ...
    BEGIN TRANSACTION
		...
		USING [qcic].[uvw_HRC_ETL06_Dim_PlayerCredit_SCD] S
		ON  T.SystemNameInt = S.SystemNameInt AND T.CasinoIDInt = S.CasinoIDInt AND T.PlayerIDBigInt = S.PlayerIDBigInt AND T.StartDateTime = S.StartDateTime AND T.IsCurrent = S.IsCurrent
		WHEN MATCHED AND 
   ...
   BEGIN TRANSACTION
		...
		USING [qcic].[uvw_HRC_ETL07_Dim_PlayerCreditElectronic_SCD] S
