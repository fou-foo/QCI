...
CREATE VIEW  [tqcic].[uvw_HRC_ETL01_Dim_Host_SCD]  AS (
SELECT 
...
CREATE VIEW [tqcic].[uvw_HRC_ETL02_Dim_Player_SCD]  AS
SELECT 
...
--------------------------
-------------- PASO 3 
CREATE VIEW [tqcic].[uvw_HRC_ETL03_Dim_PlayerDistanceToCasino_SCD]  AS (
...
-----------------------------------------------
------------ paso 4 
CREATE VIEW [tqcic].[uvw_HRC_ETL04_Dim_PlayerLoyalty_SCD]  AS
...
CREATE VIEW [tqcic].[uvw_HRC_ETL05_Dim_PlayerHost_SCD]  AS
...
---------------------
--------- paso 6 
CREATE VIEW [tqcic].[uvw_HRC_ETL06_Dim_PlayerCredit_SCD]  AS
...
----------------------------------
--------- paso 7 
CREATE VIEW [tqcic].[uvw_HRC_ETL07_Dim_PlayerCreditElectronic_SCD]  AS
SELECT	 SystemName
		,SystemNameInt
		,CasinoID
		,CasinoIDInt
...
--------------
--  paso 8 
CREATE VIEW [tqcic].[uvw_HRC_ETL08_Fact_PlayerDeviceRatingDay]  AS
--Slot Ratings
SELECT 
...
-----------------------------------
--- Paso 9 
CREATE VIEW [tqcic].[uvw_HRC_ETL09_App_DeviceManufacturerGroup]  AS
--Slot Ratings
SELECT 
		--The ETL_Runner connects qcic to qcids, so we use SystemName to track which qcic system starts the connection.
		 TRY_CONVERT(NVARCHAR(100)	,'HRC'						)								AS SystemName
...
-------------------------
-- paso 10 
CREATE VIEW [tqcic].[uvw_HRC_ETL10_App_DeviceGameTypeGroup]  AS
--Slot Ratings
SELECT 
		--The ETL_Runner connects qcic to qcids, so we use SystemName to track which qcic system starts the connection.
		 TRY_CONVERT(NVARCHAR(100)	,'HRC'						)								AS SystemName
...
----------------------
----Paso 11 
CREATE VIEW [tqcic].[uvw_HRC_ETL11_App_DeviceDenominationGroup]  AS
--Slot Ratings
SELECT 
		--The ETL_Runner connects qcic to qcids, so we use SystemName to track which qcic system starts the connection.
		 TRY_CONVERT(NVARCHAR(100)	,'HRC'					)								AS SystemName
...
--------------------------------
--Paso 12 
--drop view [tqcic].[uvw_HRC_ETL12_Fact_PlayerPrize]
CREATE VIEW [tqcic].[uvw_HRC_ETL12_Fact_PlayerPrize]  AS
--Slot Ratings
SELECT 
		--The ETL_Runner connects qcic to qcids, so we use SystemName to track which qcic system starts the connection.
		 TRY_CONVERT(NVARCHAR(100)	,'HRC'					)								AS SystemName
...
----------------------------
-- paso 14
CREATE VIEW [tqcic].[uvw_HRC_ETL14_Fact_PlayerEvaluationCurrent]  AS
--Slot Ratings
SELECT 
		--The ETL_Runner connects qcic to qcids, so we use SystemName to track which qcic system starts the connection.
		 TRY_CONVERT(NVARCHAR(100)	,'HRC'					)								AS SystemName
...
----------------------------------
-- paso 15 
CREATE VIEW [tqcic].[uvw_HRC_ETL15_Fact_DeviceDay]  AS
--Slot Ratings
SELECT 
		--The ETL_Runner connects qcic to qcids, so we use SystemName to track which qcic system starts the connection.
		 TRY_CONVERT(NVARCHAR(100)	,'HRC'						)								AS SystemName
