...
Notes:			Runs the ETLs for HRC System to build the UDM Connection Layer as well as the base UDM Data Science Layer Tables
				...
				Requires usp_HRC_ETL08_Fact_PlayerDeviceRatingDay, usp_HRC_ETL12_Fact_PlayerPrize 
				              usp_HRC_ETL15_Fact_DeviceDayudf_HRC_ETL01 to be created third
...
SELECT TimeStamp = GETDATE(),TableCompleted = convert(nvarchar(255),'Start'),RecordCount = 0 INTO [qcic].[HRC_ETL_Runner_Tracker]
  ...
  BEGIN TRANSACTION
   ...
   BEGIN TRANSACTION
    ...
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[test].[App_DeviceManufacturerGroup]',RecordCount = count(*) FROM [qcids].[App_DeviceManufacturerGroup]
		WHERE SystemNameInt = @SystemNameInt
    ...
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[test].[Fact_PlayerPrize]',RecordCount = count(*) FROM [qcids].[Fact_PlayerPrize]
		WHERE SystemNameInt = @SystemNameInt
