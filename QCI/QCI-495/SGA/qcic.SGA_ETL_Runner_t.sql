/****** Object:  StoredProcedure [tqcic].[usp_SGA_ETL_Runner]    Script Date: 10/2/2020 3:34:18 AM ******/
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tqcic].[usp_SGA_ETL_Runner]') AND type in (N'P', N'PC'))
BEGIN
	PRINT 'DROPPING [tqcic].[usp_SGA_ETL_Runner]....'
	DROP PROCEDURE [tqcic].[usp_SGA_ETL_Runner]
END
GO

PRINT 'CREATING [tqcic].[usp_SGA_ETL_Runner]....'
GO
...
***********************************************************************************************/
CREATE   PROCEDURE [tqcic].[usp_SGA_ETL_Runner] 
AS
BEGIN
	SET NOCOUNT ON;
	...
	******************/
	BEGIN TRANSACTION
		...
		--INSERT INTO [tqcids].[Dim_Host_SCD] SELECT * FROM [tqcic].[uvw_SGA_ETL01_Dim_Host_SCD]
		MERGE [tqcids].[Dim_Host_SCD] T 
		USING [tqcic].[uvw_SGA_ETL01_Dim_Host_SCD] S
   ...
   BEGIN TRANSACTION
		...
		USING [tqcic].[uvw_SGA_ETL02_Dim_Player_SCD] S
    ...
    BEGIN TRANSACTION
		...
		--INSERT INTO [tqcids].[Dim_PlayerDistanceToCasino_SCD] SELECT * FROM [tqcic].[uvw_SGA_ETL03_Dim_PlayerDistanceToCasino_SCD]
		MERGE [tqcids].[Dim_PlayerDistanceToCasino_SCD] T 
		USING [tqcic].[uvw_SGA_ETL03_Dim_PlayerDistanceToCasino_SCD] S
    ...
    BEGIN TRANSACTION
		...
		--INSERT INTO [tqcids].[Dim_PlayerLoyalty_SCD] SELECT * FROM [tqcic].[uvw_SGA_ETL04_Dim_PlayerLoyalty_SCD]
		MERGE [tqcids].[Dim_PlayerLoyalty_SCD] T
		USING [tqcic].[uvw_SGA_ETL04_Dim_PlayerLoyalty_SCD] S
		ON T.SystemNameInt = S.SystemNameInt AND T.PlayerIDBigInt = S.PlayerIDBigInt AND T.StartDateTime = S.StartDateTime AND T.IsCurrent = S.IsCurrent
		WHEN MATCHED AND 
    ...
    COMMIT TRANSACTION
     ...
     BEGIN TRANSACTION
		...
		--INSERT INTO [tqcids].[Dim_PlayerHost_SCD] SELECT * FROM [tqcic].[uvw_SGA_ETL05_Dim_PlayerHost_SCD]
		MERGE [tqcids].[Dim_PlayerHost_SCD] T 
		USING [tqcic].[uvw_SGA_ETL05_Dim_PlayerHost_SCD] S
    ...
    BEGIN TRANSACTION
		...
		--INSERT INTO [tqcids].[Dim_PlayerCredit_SCD] SELECT * FROM [tqcic].[uvw_SGA_ETL06_Dim_PlayerCredit_SCD]
		MERGE [tqcids].[Dim_PlayerCredit_SCD] T 
		USING [tqcic].[uvw_SGA_ETL06_Dim_PlayerCredit_SCD] S
		ON  T.SystemNameInt = S.SystemNameInt AND T.CasinoIDInt = S.CasinoIDInt AND T.PlayerIDBigInt = S.PlayerIDBigInt AND T.StartDateTime = S.StartDateTime AND T.IsCurrent = S.IsCurrent
		WHEN MATCHED AND 
   ...
   BEGIN TRANSACTION
		...
		--INSERT INTO [tqcids].[Dim_PlayerCreditElectronic_SCD] SELECT * FROM [tqcic].[uvw_SGA_ETL07_Dim_PlayerCreditElectronic_SCD]
		MERGE [tqcids].[Dim_PlayerCreditElectronic_SCD] T 
		USING [tqcic].[uvw_SGA_ETL07_Dim_PlayerCreditElectronic_SCD] S
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[App_DeviceManufacturerGroup] SELECT * FROM [tqcic].[uvw_SGA_ETL09_App_DeviceManufacturerGroup]
    COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[App_DeviceGameTypeGroup] SELECT * FROM [tqcic].[uvw_SGA_ETL10_App_DeviceGameTypeGroup]
    COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[App_DeviceDenominationGroup] SELECT * FROM [tqcic].[uvw_SGA_ETL11_App_DeviceDenominationGroup]
    COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[Fact_PlayerPrize] EXEC [tqcic].[usp_SGA_ETL12_Fact_PlayerPrize]  @MaxDate ,  @days 
    COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		--INSERT INTO [tqcids].[Fact_PlayerEvaluationCurrent] SELECT * FROM [tqcic].[uvw_SGA_ETL14_Fact_PlayerEvaluationCurrent]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[Fact_DeviceDay] EXEC  [tqcic].[usp_SGA_ETL15_Fact_DeviceDay] @MaxDate,  @days  
    COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[Dim_PlayerPhone_Current] SELECT * FROM [tqcic].[uvw_SGA_ETL18_Dim_PlayerPhone_Current]-- you are here 
    COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[tqcids].[Dim_PlayerPhone_Current]',RecordCount = count(*) FROM [tqcids].[Dim_PlayerPhone_Current]
		WHERE SystemNameInt = @SystemNameInt
...
RUN THIS ONCE to create indexes on the initial HRC tables
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_CMPEmployees_History] ON [tqcic].[HRC_Dim_CMPEmployees_History] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_MarketablePlayersAttribute] ON [tqcic].[HRC_Dim_MarketablePlayersAttribute] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_PlayerCreditLimit_History] ON tqcic.HRC_Dim_PlayerCreditLimit_History WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_PlayerECreditEnrollment_History] ON tqcic.HRC_Dim_PlayerECreditEnrollment_History WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_SlotAttribute_History] ON [tqcic].[HRC_Dim_SlotAttribute_History] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_SlotDenomGroup] ON tqcic.HRC_Dim_SlotDenomGroup WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_SlotGameTypeGroup] ON tqcic.HRC_Dim_SlotGameTypeGroup WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_SlotMFGGroup] ON tqcic.HRC_Dim_SlotMFGGroup WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_TableAttribute_History] ON [tqcic].[HRC_Dim_TableAttribute_History] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_DayMeterHistory] ON tqcic.HRC_Fact_DayMeterHistory WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerHostHistory_Primary] ON [tqcic].[HRC_Fact_PlayerHostHistory_Primary] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerHostHistory_Secondary] ON [tqcic].[HRC_Fact_PlayerHostHistory_Secondary] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerRedemption_Tran] ON [tqcic].[HRC_Fact_PlayerRedemption_Tran] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerSlotRating_Tran] ON [tqcic].[HRC_Fact_PlayerSlotRating_Tran] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerTableRating_Tran] ON [tqcic].[HRC_Fact_PlayerTableRating_Tran] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerValuationScore] ON tqcic.HRC_Fact_PlayerValuationScore WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
