...
/****** Object:  StoredProcedure [qcic].[usp_SGA_ETL_Runner]    Script Date: 10/7/2020 3:19:46 AM ******/
 ...
 PRINT 'START MERGE 8 ........................'
  ...
  --  BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[Fact_PlayerDeviceRatingDay] SELECT * FROM [tqcic].[uvw_SGA_ETL08_Fact_PlayerDeviceRatingDay] WHERE GamingDate >= DATEADD(DAY,-10,@MaxDate)
  --  COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
...
PRINT 'START MERGE 12 ........................'
  ...
  --  BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[Fact_PlayerPrize] SELECT * FROM [tqcic].[uvw_SGA_ETL12_Fact_PlayerPrize] WHERE PrizeStartDateTime >= DATEADD(DAY,-10,@MaxDate)
   -- COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
...
PRINT 'START MERGE 13 ........................'
   ...
   -- COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Fact_PlayerEvaluationCurrent]',RecordCount = count(*) FROM [tqcids].[Fact_PlayerEvaluationCurrent]
		WHERE SystemNameInt = @SystemNameInt
...
PRINT 'START MERGE 14 ........................'
   ...
   -- BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[Fact_DeviceDay] SELECT * FROM [tqcic].[uvw_SGA_ETL15_Fact_DeviceDay] WHERE GamingDate >= DATEADD(DAY,-10,@MaxDate)
   -- COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcids].[Fact_DeviceDay]',RecordCount = count(*) FROM [tqcids].[Fact_DeviceDay]
		WHERE SystemNameInt = @SystemNameInt
PRINT ' FINISH MERGE 14 .......................'
PRINT 'START MERGE 15 ........................'

	--DECLARE @SystemNameInt int = 2, @MaxDate DATE
   -- BEGIN TRANSACTION
		DELETE FROM [tqcids].[Dim_PlayerPhone_Current] WHERE SystemNameInt = @SystemNameInt
		INSERT INTO [tqcids].[Dim_PlayerPhone_Current] SELECT * FROM [tqcic].[uvw_SGA_ETL18_Dim_PlayerPhone_Current]
   -- COMMIT TRANSACTION
		INSERT INTO [tqcic].[SGA_ETL_Runner_Tracker]
