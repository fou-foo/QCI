/****** Object:  StoredProcedure [tqcic].[usp_HRC_ETL_Runner]    Script Date: 10/2/2020 3:34:18 AM ******/
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tqcic].[usp_HRC_ETL_Runner]') AND type in (N'P', N'PC'))
BEGIN
	PRINT 'DROPPING [tqcic].[usp_HRC_ETL_Runner]....'
	DROP PROCEDURE [tqcic].[usp_HRC_ETL_Runner]
END
GO

PRINT 'CREATING [tqcic].[usp_HRC_ETL_Runner]....'
GO
...
Notes:			Runs the ETLs for HRC System to build the UDM Connection Layer as well as the base UDM Data Science Layer Tables
				...
				Requires schemas tqcic, tqcids, and qciapi to be created first
				Requires tqcic tables to be created second
				Requires tqcic HRC_ETLXX usp to be created third
...
***********************************************************************************************/
CREATE   PROCEDURE [tqcic].[usp_HRC_ETL_Runner] -- change to [tqcic].[usp_HRC_ETL_Runner] for prod
AS
BEGIN
	SET NOCOUNT ON;
	...
	******************/
	...
	Begin
		Drop Table #temp1
	End	
	CREATE table #temp1  ( SystemName NVARCHAR(100), SystemNameInt INT, CasinoGroupID NVARCHAR(100), CasinoGroupIDInt INT, HostID NVARCHAR(100), HostIDBigInt  BIGINT, 
	...
	INSERT INTO #temp1 EXEC  [tqcic].[usp_HRC_ETL01_Dim_Player_SCD]  

	BEGIN TRANSACTION
		--DELETE FROM [qcids].[Dim_Host_SCD] WHERE SystemName = @SystemName
		--INSERT INTO [qcids].[Dim_Host_SCD] SELECT * FROM [tqcic].[uvw_HRC_ETL01_Dim_Host_SCD]
	...
	Begin
		Drop Table #temp2;
	End
	CREATE TABLE #temp2  ( SystemName  NVARCHAR(100), SystemNameInt INT, PlayerID NVARCHAR(100), PlayerIDBigInt BIGINT, Account NVARCHAR(100), Title NVARCHAR(20), 
	...
	INSERT INTO #temp2 exec  [tqcic].[usp_HRC_ETL02_Dim_Player_SCD]

   BEGIN TRANSACTION
		--DELETE FROM [qcids].[Dim_Player_SCD] WHERE SystemNameInt = @SystemNameInt
		--INSERT INTO [qcids].[Dim_Player_SCD] SELECT * FROM [tqcic].[uvw_HRC_ETL02_Dim_Player_SCD]
	...
	Begin
		Drop Table #temp3
	End
	CREATE TABLE #temp3   ( SystemName NVARCHAR(100), SystemNameInt INT, CasinoID NVARCHAR(100), CasinoIDInt INT, PlayerID NVARCHAR(100), PlayerIDBigInt BIGINT, 
	                             PlayerDistanceToCasino FLOAT, StartDateTime DATETIME , EndDateTime DATETIME, IsCurrent INT, InsertDateTime DATETIME) ; 
	INSERT INTO #temp3 EXEC  [tqcic].[usp_HRC_ETL03_Dim_PlayerDistanceToCasino_SCD]
  
    BEGIN TRANSACTION
		--DELETE FROM [qcids].[Dim_PlayerDistanceToCasino_SCD] WHERE SystemNameInt = @SystemNameInt
		--INSERT INTO [qcids].[Dim_PlayerDistanceToCasino_SCD] SELECT * FROM [tqcic].[uvw_HRC_ETL03_Dim_PlayerDistanceToCasino_SCD]
   ...
   If(OBJECT_ID('tempdb..#temp4') Is Not Null)
	Begin
		Drop Table #temp4
	End
	CREATE TABLE #temp4  (  SystemName NVARCHAR(100), SystemNameInt INT , PlayerID NVARCHAR(100), PlayerIDBigInt BIGINT, LoyaltyCardLevel NVARCHAR(100), 
									StartDateTime DATETIME, EndDateTime DATETIME, IsCurrent INT , InsertDateTime DATETIME );
	INSERT INTO #temp4 EXEC  [tqcic].[usp_HRC_ETL04_Dim_PlayerLoyalty_SCD]
  BEGIN TRANSACTION
		--DELETE FROM [qcids].[Dim_PlayerLoyalty_SCD] WHERE SystemNameInt = @SystemNameInt
		--INSERT INTO [qcids].[Dim_PlayerLoyalty_SCD] SELECT * FROM [tqcic].[uvw_HRC_ETL04_Dim_PlayerLoyalty_SCD]
	...
	Begin
		Drop Table #temp5;
	End
	CREATE TABLE #temp5  ( SystemName  NVARCHAR(100),  SystemNameInt INT,  CasinoID NVARCHAR(100), CasinoIDInt INT, PlayerID NVARCHAR(100), 
	...
	INSERT INTO #temp5 EXEC  [tqcic].[usp_HRC_ETL05_Dim_PlayerHost_SCD] 

     BEGIN TRANSACTION
		--DELETE FROM [qcids].[Dim_PlayerHost_SCD] WHERE SystemNameInt = @SystemNameInt
		--INSERT INTO [qcids].[Dim_PlayerHost_SCD] SELECT * FROM [tqcic].[uvw_HRC_ETL05_Dim_PlayerHost_SCD]
	...
	Begin
		Drop Table #temp6
	End
	CREATE table #temp6  ( SystemName NVARCHAR(100),  SystemNameInt INT, CasinoID  NVARCHAR(100), CasinoIDInt  INT, PlayerID NVARCHAR(100),  PlayerIDBigInt BIGINT,  CreditLimitInt100 INT, IsActiveCredit INT,  
	...
	INSERT INTO #temp6 EXEC [tqcic].[usp_HRC_ETL06_Dim_PlayerCredit_SCD]

    BEGIN TRANSACTION
		--DELETE FROM [qcids].[Dim_PlayerCredit_SCD] WHERE SystemNameInt = @SystemNameInt
		--INSERT INTO [qcids].[Dim_PlayerCredit_SCD] SELECT * FROM [tqcic].[uvw_HRC_ETL06_Dim_PlayerCredit_SCD]
   ...
   If(OBJECT_ID('tempdb..#temp7') Is Not Null)
	Begin
		Drop Table #temp7;
	End
	CREATE TABLE #temp7 ( SystemName NVARCHAR(100), SystemNameInt INT, CasinoID NVARCHAR(100), CasinoIDInt INT, PlayerID NVARCHAR(100), PlayerIDBigInt BIGINT,
	...
	INSERT INTO #temp7 EXEC  [tqcic].[usp_HRC_ETL07_Dim_PlayerCreditElectronic_SCD]

   BEGIN TRANSACTION
		--DELETE FROM [qcids].[Dim_PlayerCreditElectronic_SCD] WHERE SystemNameInt = @SystemNameInt
		--INSERT INTO [qcids].[Dim_PlayerCreditElectronic_SCD] SELECT * FROM [tqcic].[uvw_HRC_ETL07_Dim_PlayerCreditElectronic_SCD]
    ...
    COMMIT TRANSACTION
		INSERT INTO [tqcic].[HRC_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[tqcids].[App_DeviceManufacturerGroup]',RecordCount = count(*) FROM [tqcids].[App_DeviceManufacturerGroup]
		WHERE SystemNameInt = @SystemNameInt
    ...
    COMMIT TRANSACTION
		INSERT INTO [tqcic].[HRC_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[tqcids].[Fact_PlayerPrize]',RecordCount = count(*) FROM [tqcids].[Fact_PlayerPrize]
		WHERE SystemNameInt = @SystemNameInt

    BEGIN TRANSACTION
		--DELETE FROM [qcids].[Fact_PlayerEvaluationCurrent] WHERE SystemNameInt = @SystemNameInt
		--INSERT INTO [qcids].[Fact_PlayerEvaluationCurrent] SELECT * FROM [tqcic].[uvw_HRC_ETL14_Fact_PlayerEvaluationCurrent]
...
exec [tqcic].[usp_HRC_ETL_Runner]
select * from [tqcic].[HRC_ETL_Runner_Tracker] ORDER BY TimeStamp DESC
*/