/****** Object:  StoredProcedure [tqcic].[usp_HRC_ETL05_Dim_PlayerHost_SCD]    Script Date: 10/22/2020 5:29:30 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
	CREATE OR ALTER   PROCEDURE  [tqcic].[usp_HRC_ETL05_Dim_PlayerHost_SCD]
	AS
		BEGIN
		SET NOCOUNT ON;
        WITH temp as (
	        SELECT	 	
                ...
                FROM	 [tqcic].[HRC_Fact_PlayerHostHistory_Primary] a WITH (NOLOCK)
            ...
            SELECT	 	
		        ...
		        ,TRY_CONVERT(INT			,isCurrent				)		AS IsCurrent
		        ,GETUTCDATE()												AS InsertDateTime
                FROM	 [tqcic].[HRC_Fact_PlayerHostHistory_Secondary] a WITH (NOLOCK)
                JOIN	 [qcids].[App_Casino] b WITH (NOLOCK)
                ON		(a.CasinoID = b.CasinoID AND b.SystemNameInt = 3)
                GROUP BY a.CasinoID, b.CasinoIDInt, PlayerID, HostEmpID, HostStartDTM, HostEndDTM, isCurrent ), 
        fix_temp AS (
