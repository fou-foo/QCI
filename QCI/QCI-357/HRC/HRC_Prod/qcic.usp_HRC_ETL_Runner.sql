/****** Object:  StoredProcedure [qcic].[usp_HRC_ETL_Runner]    Script Date: 10/2/2020 3:34:18 AM ******/
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[qcic].[usp_HRC_ETL_Runner]') AND type in (N'P', N'PC'))
BEGIN
	PRINT 'DROPPING [qcic].[usp_HRC_ETL_Runner]....'
	DROP PROCEDURE [qcic].[usp_HRC_ETL_Runner]
END
GO

PRINT 'CREATING [qcic].[usp_HRC_ETL_Runner]....'
GO
...
Notes:			Runs the ETLs for HRC System to build the UDM Connection Layer as well as the base UDM Data Science Layer Tables
				...
				Requires qcic HRC_ETLXX usp to be created third
				This gets run fourth				
				
Modifictions:
Date			Name			Notes
2020-10-22	    Antonio Garcia  Add [usp_HRC_ETL01_Dim_Player_SCD] to clean [qcids].[Dim_Host_SCD] table
                                Add [usp_HRC_ETL02_Dim_Player_SCD] to clean [qcids].[Dim_Player_SCD] table
								Add [usp_HRC_ETL03_Dim_PlayerDistanceToCasino_SCD] to clean [qcids].[Dim_PlayerDistanceToCasino_SCD] table
								Add [usp_HRC_ETL04_Dim_PlayerLoyalty_SCD] to clean [qcids].[Dim_PlayerLoyalty_SCD] table
								Add [usp_HRC_ETL05_Dim_PlayerHost_SCD]  to clean [qcids].[Dim_PlayerHost_SCD] table
								Add [usp_HRC_ETL06_Dim_PlayerCredit_SCD] to clean [qcids].[Dim_PlayerCredit_SCD] table
								Add [usp_HRC_ETL07_Dim_PlayerCreditElectronic_SCD] to clean [qcids].[Dim_PlayerCreditElectronic_SCD] table
***********************************************************************************************/
CREATE   PROCEDURE [qcic].[usp_HRC_ETL_Runner] -- change to [qcic].[usp_HRC_ETL_Runner] for prod
AS
BEGIN
	SET NOCOUNT ON;
	DECLARE @SystemName nvarchar(100) = 'HRC', @SystemNameInt int = 3 , @MaxDate date

	DROP TABLE IF EXISTS [qcic].[HRC_ETL_Runner_Tracker]
	SELECT TimeStamp = GETDATE(),TableCompleted = convert(nvarchar(255),'Start'),RecordCount = 0 INTO [qcic].[HRC_ETL_Runner_Tracker]

	/******************
	UDM-DS Steps
	******************/
    If(OBJECT_ID('tempdb..#temp1_HRC') Is Not Null)
	Begin
		Drop Table #temp1_HRC
	End	
	CREATE table #temp1_HRC  ( SystemName NVARCHAR(100), SystemNameInt INT, CasinoGroupID NVARCHAR(100), CasinoGroupIDInt INT, HostID NVARCHAR(100), HostIDBigInt  BIGINT, 
	...
	INSERT INTO #temp1_HRC EXEC  [qcic].[usp_HRC_ETL01_Dim_Player_SCD]  
	...
	BEGIN TRANSACTION
		...
		WHEN NOT MATCHED BY SOURCE AND T.SystemNameInt = @SystemNameInt
			THEN DELETE;
	COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker]
		SELECT TimeStamp = GETDATE(),TableCompleted = '[qcic].[Dim_Host_SCD]',RecordCount = count(*) FROM [qcids].[Dim_Host_SCD]
		WHERE SystemNameInt = @SystemNameInt

	If(OBJECT_ID('tempdb..#temp2') Is Not Null)
	Begin
		Drop Table #temp2_HRC;
	End
	CREATE TABLE #temp2_HRC  ( SystemName  NVARCHAR(100), SystemNameInt INT, PlayerID NVARCHAR(100), PlayerIDBigInt BIGINT, Account NVARCHAR(100), Title NVARCHAR(20), 
	...
	INSERT INTO #temp2_HRC exec  [qcic].[usp_HRC_ETL02_Dim_Player_SCD]
   ...
   BEGIN TRANSACTION
	...
	If(OBJECT_ID('tempdb..#temp3_HRC') Is Not Null)
	Begin
		Drop Table #temp3_HRC
	End
	CREATE TABLE #temp3_HRC   ( SystemName NVARCHAR(100), SystemNameInt INT, CasinoID NVARCHAR(100), CasinoIDInt INT, PlayerID NVARCHAR(100), PlayerIDBigInt BIGINT, 
	                             PlayerDistanceToCasino FLOAT, StartDateTime DATETIME , EndDateTime DATETIME, IsCurrent INT, InsertDateTime DATETIME) ; 
	INSERT INTO #temp3_HRC EXEC  [qcic].[usp_HRC_ETL03_Dim_PlayerDistanceToCasino_SCD]
   ...
   If(OBJECT_ID('tempdb..#temp4_HRC') Is Not Null)
	Begin
		Drop Table #temp4_HRC
	End
	CREATE TABLE #temp4_HRC  (  SystemName NVARCHAR(100), SystemNameInt INT , PlayerID NVARCHAR(100), PlayerIDBigInt BIGINT, LoyaltyCardLevel NVARCHAR(100), 
									StartDateTime DATETIME, EndDateTime DATETIME, IsCurrent INT , InsertDateTime DATETIME );
	INSERT INTO #temp4_HRC EXEC  [qcic].[usp_HRC_ETL04_Dim_PlayerLoyalty_SCD]
  BEGIN TRANSACTION
    ...
    If(OBJECT_ID('tempdb..#temp5_HRC') Is Not Null)
	Begin
		Drop Table #temp5_HRC;
	End
	CREATE TABLE #temp5_HRC  ( SystemName  NVARCHAR(100),  SystemNameInt INT,  CasinoID NVARCHAR(100), CasinoIDInt INT, PlayerID NVARCHAR(100), 
	...
	INSERT INTO #temp5_HRC EXEC  [qcic].[usp_HRC_ETL05_Dim_PlayerHost_SCD] 
    ...
    if(OBJECT_ID('tempdb..#temp6_HRC') Is Not Null)
	Begin
		Drop Table #temp6_HRC
	End
	CREATE table #temp6_HRC  ( SystemName NVARCHAR(100),  SystemNameInt INT, CasinoID  NVARCHAR(100), CasinoIDInt  INT, PlayerID NVARCHAR(100),  PlayerIDBigInt BIGINT,  CreditLimitInt100 INT, IsActiveCredit INT,  
	...
	INSERT INTO #temp6_HRC EXEC [qcic].[usp_HRC_ETL06_Dim_PlayerCredit_SCD]
   ...
   If(OBJECT_ID('tempdb..#temp7_HRC') Is Not Null)
	Begin
		Drop Table #temp7_HRC;
	End
	CREATE TABLE #temp7_HRC ( SystemName NVARCHAR(100), SystemNameInt INT, CasinoID NVARCHAR(100), CasinoIDInt INT, PlayerID NVARCHAR(100), PlayerIDBigInt BIGINT,
	...
	INSERT INTO #temp7_HRC EXEC  [qcic].[usp_HRC_ETL07_Dim_PlayerCreditElectronic_SCD]
   ...
   BEGIN TRANSACTION
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [qcids].[Fact_PlayerDeviceRatingDay] EXEC [qcic].[usp_HRC_ETL08_Fact_PlayerDeviceRatingDay] @MaxDate, @days
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [qcids].[App_DeviceManufacturerGroup] SELECT * FROM  [qcic].[uvw_HRC_ETL09_App_DeviceManufacturerGroup]
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [qcids].[App_DeviceGameTypeGroup] SELECT * FROM  [qcic].[uvw_HRC_ETL10_App_DeviceGameTypeGroup]
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [qcids].[App_DeviceDenominationGroup] SELECT * FROM  [qcic].[uvw_HRC_ETL11_App_DeviceDenominationGroup]
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [qcids].[Fact_PlayerPrize]  EXEC [qcic].[usp_HRC_ETL12_Fact_PlayerPrize] @MaxDate, @days
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker]
    ...
    BEGIN TRANSACTION
		...
		USING [qcic].[uvw_HRC_ETL14_Fact_PlayerEvaluationCurrent] S
    ...
    BEGIN TRANSACTION
		...
		INSERT INTO [qcids].[Fact_DeviceDay] EXEC  [qcic].[usp_HRC_ETL15_Fact_DeviceDay] @MaxDate, @days
    COMMIT TRANSACTION
		INSERT INTO [qcic].[HRC_ETL_Runner_Tracker]
...
/*
RUN THIS ONCE to create indexes on the initial HRC tables
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_CMPEmployees_History] ON [qcic].[HRC_Dim_CMPEmployees_History] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_MarketablePlayersAttribute] ON [qcic].[HRC_Dim_MarketablePlayersAttribute] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_PlayerCreditLimit_History] ON qcic.HRC_Dim_PlayerCreditLimit_History WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_PlayerECreditEnrollment_History] ON qcic.HRC_Dim_PlayerECreditEnrollment_History WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_SlotAttribute_History] ON [qcic].[HRC_Dim_SlotAttribute_History] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_SlotDenomGroup] ON qcic.HRC_Dim_SlotDenomGroup WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_SlotGameTypeGroup] ON qcic.HRC_Dim_SlotGameTypeGroup WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_SlotMFGGroup] ON qcic.HRC_Dim_SlotMFGGroup WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Dim_TableAttribute_History] ON [qcic].[HRC_Dim_TableAttribute_History] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_DayMeterHistory] ON qcic.HRC_Fact_DayMeterHistory WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerHostHistory_Primary] ON [qcic].[HRC_Fact_PlayerHostHistory_Primary] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerHostHistory_Secondary] ON [qcic].[HRC_Fact_PlayerHostHistory_Secondary] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerRedemption_Tran] ON [qcic].[HRC_Fact_PlayerRedemption_Tran] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerSlotRating_Tran] ON [qcic].[HRC_Fact_PlayerSlotRating_Tran] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerTableRating_Tran] ON [qcic].[HRC_Fact_PlayerTableRating_Tran] WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
	CREATE CLUSTERED COLUMNSTORE INDEX [cci_qcic_HRC_Fact_PlayerValuationScore] ON qcic.HRC_Fact_PlayerValuationScore WITH (DROP_EXISTING = OFF, COMPRESSION_DELAY = 0) ON [PRIMARY];
exec [qcic].[usp_HRC_ETL_Runner]
select * from [qcic].[HRC_ETL_Runner_Tracker] ORDER BY TimeStamp DESC
*/