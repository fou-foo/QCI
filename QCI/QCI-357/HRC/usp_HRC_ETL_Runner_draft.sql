...
SELECT TimeStamp = GETDATE(),TableCompleted = convert(nvarchar(255),'Start'),RecordCount = 0 INTO [tqcic].[HRC_ETL_Runner_Tracker]
  ...
  -- BEGIN TRANSACTION
   ...
   -- BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[Fact_PlayerDeviceRatingDay] SELECT * FROM  [tqcic].[uvw_HRC_ETL08_Fact_PlayerDeviceRatingDay] WHERE GamingDate >= DATEADD(DAY,-10,@MaxDate)
  --  COMMIT TRANSACTION
		INSERT INTO [tqcic].[HRC_ETL_Runner_Tracker]
...
--    COMMIT TRANSACTION
    ...
    --BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[Fact_PlayerPrize] SELECT * FROM  [tqcic].[uvw_HRC_ETL12_Fact_PlayerPrize] WHERE PrizeStartDateTime >= DATEADD(DAY,-10,@MaxDate)
   -- COMMIT TRANSACTION
		INSERT INTO [tqcic].[HRC_ETL_Runner_Tracker]
   ...
   -- BEGIN TRANSACTION
		...
		INSERT INTO [tqcids].[Fact_DeviceDay] SELECT * FROM  [tqcic].[uvw_HRC_ETL15_Fact_DeviceDay] WHERE GamingDate >= DATEADD(DAY,-10,@MaxDate)
   -- COMMIT TRANSACTION
		INSERT INTO [tqcic].[HRC_ETL_Runner_Tracker]
